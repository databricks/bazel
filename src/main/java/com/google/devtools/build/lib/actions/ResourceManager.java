...
public class ResourceManager implements ResourceEstimator {
  ...
  @AutoValue
  abstract static class ResourceRequest {
   static ResourceRequest create(ActionExecutionMetadata getOwner, ResourceSet getResourceSet, ResourcePriority getPriority, int getId) {
     return new AutoValue_ResourceManager_ResourceRequest(getOwner, getResourceSet, getPriority, getId);
   }
   abstract ActionExecutionMetadata getOwner();
   abstract ResourceSet getResourceSet();
   abstract ResourcePriority getPriority();
   abstract int getId();
  } 
  /*
  Use AutoValue to simulate this record
  ...
  */
  ...
  public ResourceHandle acquireResources(
    ...
    ResourceRequest request =
        ResourceRequest.create(owner, resources, priority, requestIdGenerator.getAndIncrement());
  ...
  synchronized boolean areResourcesAvailable(ResourceSet resources) {
    ...
    if (workerKey != null) {
      ...
      if (activeWorkers >= availableWorkers || !workerPool.couldBeBorrowed(workerKey)) {
        return false;
      }
    }
