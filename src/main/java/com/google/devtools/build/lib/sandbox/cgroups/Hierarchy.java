...
public abstract class Hierarchy {
    ...
    public boolean isV2() {
        return controllers().isEmpty() && id() == 0;
    }
    ...
    static Hierarchy create(Integer id, List<String> controllers, String path) {
        return new AutoValue_Hierarchy(id, controllers, Paths.get(path));
    }
    ...
    static List<Hierarchy> parse(File procCgroup) throws IOException {
        ...
        for (String line : Files.readLines(procCgroup, StandardCharsets.UTF_8)) {
            ...
            hierarchies.add(Hierarchy.create(id, cs, path));
        }
